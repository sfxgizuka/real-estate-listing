const PropertyDetail = ({ property }: { property: any }) => {
    ...
    return (
      <div className="p-4">
        ...
        />
        <p className="text-xl mb-2">{property.last_price_change_amount ? `$${property?.last_price_change_amount}` : 'Price not available'}</p>
        <p className="mb-2">{property.location?.address?.city}, {property.location?.address?.state_code} {property.location?.address?.postal_code}</p>
        <p className="mb-4">{typeof property.description?.text === 'string' ? property.description?.text : 'No description available'}</p>
